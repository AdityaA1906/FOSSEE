...
def gauss_jacobi(A, b, tolerance=0.0001, max_iterations=100):
    n = len(b)
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        print("y(k+1) = (b2 - a21*x(k) - a23*z(k)) / a22")
        print("z(k+1) = (b3 - a31*x(k) - a32*y(k)) / a33")
        print("-" * 80)
        
        print(f"Current values (from prev iteration): x = {x[0]:.4f}, y = {x[1]:.4f}, z = {x[2]:.4f}\n")
        
        # Calculate new values using old values
        for i in range(n):
            sum_val = b[i]
            formula_parts = [f"b{i+1}"]
            
            for j in range(n):
                if i != j:
                    sum_val -= A[i][j] * x[j]
                    formula_parts.append(f"a{i+1}{j+1}*{['x', 'y', 'z'][j]}({iteration-1})")
            ...
            for j in range(n):
                if i != j:
                    calc_str += f" - {A[i][j]:.4f}*{x[j]:.4f}"
            calc_str += f") / {A[i][i]:.4f}"
            print(calc_str)
            print(f"{var_name}({iteration}) = {x_new[i]:.4f}\n")
        ...
        if error < tolerance:
            ...
            print(f"{'=' * 80}")
            print(f"x = {x_new[0]:.4f}")
            print(f"y = {x_new[1]:.4f}")
            print(f"z = {x_new[2]:.4f}")
            print(f"{'=' * 80}")
